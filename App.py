...
def fetch_news_poster(poster_link):
    try:
        ...
        image = Image.open('Data/no_image.jpeg')
        st.image(image, use_column_width=True)
...
def run():
    st.title("News++ :newspaper: : A Summarised News Web App")
    st.subheader("Made by : Ligandro")
    image = Image.open('Data/newspaper.jpeg')
    ...
    if cat_op == category[0]:
        ...
        av_topics = ['--Select--','WORLD', 'NATION', 'BUSINESS', 'TECHNOLOGY', 'ENTERTAINMENT', 'SPORTS', 'SCIENCE',
                     'HEALTH']
        st.subheader("Choose your favourite Topic")
        chosen_topic = st.sidebar.selectbox("Choose your favourite Topic", av_topics)
        if chosen_topic == av_topics[0]:
            st.sidebar.warning("Please Choose the Topic")
        else:
            no_of_news = st.sidebar.slider('Number of News:', min_value=5, max_value=25, step=1)
        ...
        if st.sidebar.button("Search") and user_topic != '':
            ...
            st.sidebar.warning("Please write Topic Name to Search🔍")
